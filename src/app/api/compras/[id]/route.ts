...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!compra) return NextResponse.json({ error: 'No encontrada' }, { status: 404 })
  if (compra.estado === 'ANULADA') return NextResponse.json({ error: 'Ya está anulada' }, { status: 400 })
    ...
    if (compra.estado === 'CONFIRMADA') {
      for (const d of compra.detalles) {
        ...
        if (articulo?.precioCompra && d.cantidad > 0) {
          ...
          const stockTotal = stocksActualizados.reduce((a, s) => a + s.cantidad, 0)

          if (stockTotal > 0) {
            // Recalcular precio inverso
            const precioRevertido = (
              (articulo.precioCompra * (stockTotal + d.cantidad)) - (d.precioUnitario * d.cantidad)
            ) / stockTotal
