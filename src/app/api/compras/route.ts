...
export async function POST(req: NextRequest) {
  ...
  const detallesCalculados = detalles.map((d: any) => {
    ...
    if (d.impuestoId) {
      ...
      if (pct === 5) {
        ivaItem = subtotalItem / 21;
        totalIva5 += ivaItem;
      }
      if (pct === 10) {
        ivaItem = subtotalItem / 11;
        totalIva10 += ivaItem;
      }
    }
  ...
  const compra = await prisma.$transaction(async (tx) => {
    ...
    if (estado === "CONFIRMADA" && almacenId) {
      for (const d of detallesCalculados) {
        ...
        if (articulo?.inventariable) {
          ...
          const nuevoPrecio =
            (cantidadActual * precioActual + d.cantidad * d.precioUnitario) /
    ...
    await registrarAuditoria({
      ...
      descripcion: `Nueva compra ${compra.nroComprobante || compra.id} — Gs. ${compra.total}`,
      metadata: {
        compraId: compra.id,
