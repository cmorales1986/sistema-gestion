// src/app/api/ventas/cobros/route.ts
...
export async function POST(req: NextRequest) {
  ...
  const resultado = await prisma.$transaction(async (tx) => {
    ...
    if (crearMovBancario) {
      const concepto = body.medioPago === 'CHEQUE'
        ? `Cheque ${body.nroReferencia ? '#' + body.nroReferencia + ' — ' : ''}Cobro factura ${venta.nroComprobante || body.ventaId}`
        : `Transferencia${body.nroReferencia ? ' ' + body.nroReferencia : ''} — Cobro factura ${venta.nroComprobante || body.ventaId}`
  ...
  await registrarAuditoria({
    ...
    descripcion: `Cobro de Gs. ${monto} — ${body.medioPago} — venta ${venta.nroComprobante || body.ventaId}`,
    metadata: {
      cobroId:          resultado.id,
      ventaId:          body.ventaId,
      monto,
