...
export default function ProveedoresPage() {
  ...
  const cargar = useCallback(async () => {
    setLoading(true)
    const res = await fetch(`/api/proveedores?q=${busqueda}`)
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{proveedores.length} proveedor{proveedores.length !== 1 ? 'es' : ''} registrado{proveedores.length !== 1 ? 's' : ''}</p>
        </div>
        <button
          onClick={abrirCrear}
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                { name: 'email',     label: 'Email',       placeholder: 'proveedor@email.com' },
