...
export default function PagosComprasPage() {
  ...
  const [filtros, setFiltros] = useState({ desde: inicioMes, hasta: hoy, proveedorId: '', medioPago: '' })
  ...
  // Cargar compras pendientes de pago al abrir drawer
  async function abrirDrawer() {
    ...
    setForm({ compraId: '', fecha: hoy, monto: '', medioPago: 'EFECTIVO', nroReferencia: '', observacion: '' })
    setError('')
    setDrawerOpen(true)
  }

  // Al seleccionar compra, autocompletar monto con el saldo
  ...
  async function eliminar(id: string) {
    ...
    await fetch(`/api/compras/pagos/${id}`, { method: 'DELETE' })
    cargar()
  }
    ...
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-sm mt-0.5">{pagos.length} pago{pagos.length !== 1 ? 's' : ''} registrado{pagos.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-4">
        <div className="grid grid-cols-5 gap-3">
          ...
          <div>
            ...
            <select value={filtros.proveedorId} onChange={e => setFiltros({ ...filtros, proveedorId: e.target.value })}
            ...
            <tbody>
              {pagos.map((p, i) => (
                <tr key={p.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-50/50'}`}>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => eliminar(p.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <select value={form.compraId} onChange={e => seleccionarCompra(e.target.value)}
