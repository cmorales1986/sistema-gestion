...
export default function ReporteVentasPage() {
  ...
  function exportarVentas() {
    ...
    const filas = data.ventas.map((v) => ({
      ...
      Condición: v.condicionPago?.nombre || "—",
      Subtotal: v.subtotal,
      "IVA 5%": v.totalIva5,
      "IVA 10%": v.totalIva10,
      Total: v.total,
      Cobrado: v.montoPagado,
      Pendiente: v.total - v.montoPagado,
      "Estado cobro": v.estadoPago,
    }));
    ...
    filas.push({
      ...
      Cliente: "",
      Condición: "",
      Subtotal: data.ventas.reduce((a, v) => a + v.subtotal, 0),
      "IVA 5%": data.totalIva5,
      "IVA 10%": data.totalIva10,
      Total: data.totalGeneral,
      Cobrado: data.totalCobrado,
      Pendiente: data.totalPendiente,
      "Estado cobro": "",
    });

    exportarExcel(filas, `ventas-${filtros.desde}-${filtros.hasta}`, "Ventas");
  }
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            {data?.ventas.length || 0} registros encontrados
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={exportarVentas}
            disabled={!data?.ventas.length}
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-40"
          >
          ...
          <button
            ...
            className="no-print flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors"
          >
          ...
          <table className="w-full">
              ...
              {data.ventas.map((v, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">
                    {v.condicionPago?.nombre || "—"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-700 text-right">
                    Gs. {formatGs(v.subtotal)}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-700 text-right">
                    {v.totalIva5 > 0 ? `Gs. ${formatGs(v.totalIva5)}` : "—"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-700 text-right">
                    {v.totalIva10 > 0 ? `Gs. ${formatGs(v.totalIva10)}` : "—"}
                  </td>
            ...
            <tfoot>
              <tr className="border-t-2 border-gray-200 bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm font-bold text-gray-900 text-right">
                  Gs.{" "}
                  {formatGs(data.ventas.reduce((a, v) => a + v.subtotal, 0))}
                </td>
                <td className="px-4 py-3 text-sm font-bold text-gray-900 text-right">
                  Gs. {formatGs(data.totalIva5)}
                </td>
                <td className="px-4 py-3 text-sm font-bold text-gray-900 text-right">
                  Gs. {formatGs(data.totalIva10)}
                </td>
                <td className="px-4 py-3 text-sm font-bold text-gray-900 text-right">
                  Gs. {formatGs(data.totalGeneral)}
                </td>
                <td />
              </tr>
            </tfoot>
