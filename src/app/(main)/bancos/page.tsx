...
import { Plus, Banknote, CreditCard, ArrowRight, TrendingUp, TrendingDown } from 'lucide-react'
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{cuentas.length} cuenta{cuentas.length !== 1 ? 's' : ''} bancaria{cuentas.length !== 1 ? 's' : ''}</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 col-span-1">
          ...
          <p className="text-2xl font-bold text-gray-900">Gs. {formatGs(saldoTotal)}</p>
          <p className="text-xs text-gray-400 mt-1">{cuentas.length} cuenta{cuentas.length !== 1 ? 's' : ''}</p>
        </div>
        <Link href="/bancos/cheques"
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Gestión de cheques emitidos y recibidos</p>
          </div>
        ...
        <Link href="/bancos/movimientos"
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Historial de movimientos bancarios</p>
          </div>
          <ArrowRight className="w-4 h-4 text-gray-400 ml-auto" />
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border-2 border-dashed border-gray-200 p-16 text-center">
          ...
          <p className="text-gray-500 font-medium mb-2">No hay cuentas bancarias</p>
          <p className="text-gray-400 text-sm mb-6">Agregá tu primera cuenta bancaria para empezar</p>
          <Link href="/bancos/cuentas/nueva"
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{c.banco.nombre}</p>
                    <p className="text-xs text-gray-400 font-mono">{c.nroCuenta}</p>
                  </div>
                </div>
              ...
              <div className="flex items-end justify-between pt-3 border-t border-gray-100">
                <div>
                  ...
                  <p className={`text-xl font-bold ${c.saldoActual >= 0 ? 'text-gray-900' : 'text-red-600'}`}>
                    {c.moneda?.simbolo || 'Gs.'} {formatGs(c.saldoActual)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-gray-400">Saldo inicial</p>
                  <p className="text-sm text-gray-500">Gs. {formatGs(c.saldoInicial)}</p>
                </div>
              </div>
            </Link>
          ))}
