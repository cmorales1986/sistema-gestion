...
type Articulo      = { id: string; codigo: string | null; nombre: string; unidadMedida: string; impuestoId: string | null; precioVenta: number | null }
type CondicionPago = { id: string; nombre: string; dias: number }
type Moneda        = { id: string; codigo: string; nombre: string; simbolo: string; principal: boolean }
type Impuesto      = { id: string; nombre: string; porcentaje: number }
type Timbrado      = { id: string; numero: string; serie1: string; serie2: string; siguiente: number; hasta: number }
    ...
    fetch('/api/timbrados/activo')
      ...
      .then(data => {
        if (data.timbrado && data.nroFormateado) {
          ...
          setCabecera(prev => ({ ...prev, nroComprobante: data.nroFormateado }))
        }
      })
  }, [])
