...
export default function Header() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/empresas")
      ...
      .then((data) => {
        setEmpresas(data);
        // Restaurar empresa activa del localStorage
        ...
        {rol === "SUPERADMIN" && (
          <div className="relative mr-2" ref={empresaRef}>
            ...
            >
              ...
              <ChevronDown
                className={`w-3 h-3 transition-transform ${empresaMenuOpen ? "rotate-180" : ""}`}
              />
            </button>
              ...
              <div className="absolute right-0 top-10 w-72 bg-white rounded-xl border border-gray-200 shadow-lg z-50 overflow-hidden">
                <div className="p-3 border-b border-gray-100">
                  ...
                  <input
                    autoFocus
                    value={busqEmpresa}
                    onChange={(e) => setBusqEmpresa(e.target.value)}
                    ...
                    empresasFiltradas.map((e) => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 ${
                          empresaActiva?.id === e.id ? "bg-blue-50" : ""
                        }`}
                      >
                        ...
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_COLOR[e.estado] || "bg-gray-100 text-gray-600"}`}
                        >
                          {e.estado}
                        </span>
                      </button>
              ...
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                {totalNotis > 0 && (
                  <span className="text-xs text-gray-500">
                    {totalNotis} alerta{totalNotis !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
                ...
                {notificaciones.length === 0 ? (
                  ...
                  notificaciones.map((n) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${n.nivel === "ERROR" ? "bg-red-100" : "bg-orange-100"}`}
                        >
                          <Icon
                            className={`w-3.5 h-3.5 ${n.nivel === "ERROR" ? "text-red-600" : "text-orange-600"}`}
                          />
                        </div>
